...
func similarity(file1 []int, target []int) float64 {
	targetCopy := make([]int, len(target))
	copy(targetCopy, target)

	counter := 0

	for _, value := range file1 {
		for i, targetValue := range targetCopy {
	...
	result := float64(counter) / float64(len(file1))
	return result
}
